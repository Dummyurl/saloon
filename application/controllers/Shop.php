...
class Shop extends CI_Controller {
	...
	public function view()
	{
		...
		$template['sid'] = $params[count($params)-1];
		if(count($params) > 1) {
	...
	public function addtocart()
	{
		...
		$data['shop_service_id'] = $this->input->post('shop_service_id');
		$data['sub_service_name'] = $this->input->post('sub_service_name');
		$data['sub_service_price'] = $this->input->post('sub_service_price');
		$cart_data = array(
       		 'id'      => $data['shop_service_id'],
       		 'qty'     => 1,
       		 'price'   => $data['sub_service_price'],
       		 'name'    => $data['sub_service_name']
			);
	...
	public function book()
	{
		...
		$id = $this->uri->segment(3);
		$service_name = $this->uri->segment(4);
		if($service_name){
		...
		$template['servicename']=$service_name;
		$template['id'] = $id;
		$template['data'] = $result;
		/*var_dump($result);
		exit;*/
...
public function get_staff_list(){
	...
	$url = $this->config->item('webservice_url')."getstafflist";
	$curl_handle = curl_init();
	curl_setopt($curl_handle, CURLOPT_HTTPHEADER, array('X-API-KEY: '.get_security_key()));
	curl_setopt($curl_handle, CURLOPT_URL, $url);
	curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
